...
func Parse(userAgent string) UserAgent {
	...
	case tokens.exists(Bingbot):
		ua.Name = Bingbot
		ua.Version = tokens.get(Bingbot)
		ua.Bot = true
		ua.Mobile = tokens.existsAny("Mobile", "Mobile Safari")

	case tokens.exists(Baidubot):
		ua.Name = Baidubot
		ua.Version = tokens.get(Baidubot)
		ua.Bot = true
		ua.Mobile = tokens.existsAny("Mobile", "Mobile Safari")

	case tokens.exists(Sobot):
		ua.Name = Sobot
		ua.Version = tokens.get(Sobot)
		ua.Bot = true
		ua.Mobile = tokens.existsAny("Mobile", "Mobile Safari")

	case tokens.exists(Sogoubot):
		ua.Name = Sogoubot
		ua.Version = tokens.get(Sogoubot)
		ua.Bot = true
		ua.Mobile = tokens.existsAny("Mobile", "Mobile Safari")
	...
	case tokens.get(OperaTouch) != "":
		ua.Name = OperaTouch
		ua.Version = tokens.get(OperaTouch)
		ua.Tablet = true

	case tokens.get(Opera) != "":
		ua.Name = Opera
		ua.Version = tokens.get(Opera)
		ua.Mobile = tokens.existsAny("Mobile", "Mobile Safari")
...
func (p properties) findBestMatch(withVerOnly bool) string {
	...
	for i := 0; i < n; i++ {
		for _, prop := range p.list {
			switch prop.Key {
			case Chrome, Firefox, Safari, "Version", "Mobile", "Mobile Safari", "Mozilla", "AppleWebKit", "Windows NT", "Windows Phone OS", Android, "Macintosh", Linux, "GSA", "CrOS", "Tablet", Opera, Edge, ChromeOS:
			default:
...
func (p *properties) findAndroidDevice(startIndex int) string {
	for i := startIndex; i < startIndex+1; i++ {
		if len(p.list) > i+1 {
			...
			switch dev {
			case Chrome, Firefox, Safari, "Opera Mini", "Presto", "Version", "Mobile", "Mobile Safari", "Mozilla", "AppleWebKit", "Windows NT", "Windows Phone OS", Android, "Macintosh", Linux, "CrOS", Opera, Edge, ChromeOS:
